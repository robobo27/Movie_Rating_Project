...
def covariance(a):
    """
    returns a new covariance matrix, changed to be 2x2 for 2 inputs
    """
    meanA = mean(a)
    total = [[0,0],[0,0]]
    for r in range(len(a)):
...
def discriminant(class1,class2,class3,vector):
    ...
    #class 1 function
    v1 = [vector[0] - class1mean[0], vector[1] - class1mean[1]]#changed rfor 2
    v1trans = transpose(v1)
    v2 = multiply(v1,class1inverse)
    g1vector = multiply(v2,v1trans)
    ...
    #class 2 function
    v3 = [vector[0] - class2mean[0], vector[1] - class2mean[1]]#changed rfor 2
    v2trans = transpose(v3)
    v4 = multiply(v3,class2inverse)
    g2vector = multiply(v4,v2trans)
    rest2 = -.5*(g2vector[0]) - g2m + math.log(len(class2)/length)

    # class 3 function
    v5 = [vector[0] - class3mean[0], vector[1] - class3mean[1]]#changed rfor 2
    v3trans = transpose(v5)
    v6 = multiply(v5,class3inverse)
    g3vector = multiply(v6,v3trans)
    rest3 = -.5*(g3vector[0]) - g3m + math.log(len(class3)/length)
